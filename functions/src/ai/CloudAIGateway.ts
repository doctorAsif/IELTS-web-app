import { CloudAIAdapter } from './adapters/CloudAIAdapter';
...
export class CloudAIGateway {
  private static instance: CloudAIGateway;
  private adapters: Map<string, CloudAIAdapter>;

  private constructor() {
    this.adapters = new Map();
    this.adapters.set('gemini', new GeminiProvider());
    this.adapters.set('openai', new OpenAIProvider());
    this.adapters.set('claude', new ClaudeProvider());
  }

  public static getInstance(): CloudAIGateway {
  ...
  public async execute(taskType: string, payload: any, context?: any): Promise<any> {
    ...
    if (context?.preferredProvider && settings.enabledProviders.includes(context.preferredProvider)) {
    ...
    return {
      ...
      estimatedTokens: 150, // Mock token count
