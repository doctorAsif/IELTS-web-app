...
export const DashboardView: React.FC<DashboardViewProps> = ({ onStartLesson, onSelectTab }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 flex flex-col xl:flex-row gap-8">
      {/* Main Content */}
      ...
      <div className="hidden xl:block w-80 shrink-0">
        <div className="sticky top-6">
          <img 
            src="/ad-banner.jpg" 
            alt="Advertisement" 
            className="w-full rounded-2xl shadow-lg border-4 border-[#1E293B]"
          />
        </div>
      </div>

      {/* Side Banner (Mobile/Tablet) */}
      <div className="block xl:hidden mt-6">
        <img 
          src="/ad-banner.jpg" 
          alt="Advertisement" 
          className="w-full max-w-sm mx-auto rounded-2xl shadow-lg border-4 border-[#1E293B]"
        />
      </div>
    </div>
  );
